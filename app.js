    ...
    for(i in bans) {
        if(bans[i][0] == clients[conn.id].ip) {
            if(Date.now() < bans[i][2]) {
                conn.write(JSON.stringify({type:'server', info:'rejected', reason:'banned', time:Math.round(bans[i][2] - Date.now())}));
                return conn.close();
            } else {
                bans.splice(i);
            }
    ...
    conn.on('data', function(message) {
        ...
        if(rateLimit[conn.id] < 1 && JSON.parse(message).type != 'delete' && JSON.parse(message).type != 'typing' && JSON.parse(message).type != 'ping') {
            ...
            try {
                ...
                if(data.type == 'message' && clients[conn.id].muted == true) {
                  /*
 return conn.write(JSON.stringify({type:'server', info:'rejected', reason:'muted', time:"2" ,keep:true,prettyms: prettyMilliseconds(clients[conn.id].timeMute - Date.now(), {verbose: true})
}));
*/

 for(i in muted) {
        if(muted[i][0] == clients[conn.id].ip) {
            if(Date.now() < muted[i][2]) {
              return  conn.write(JSON.stringify({type:'server', info:'rejected', reason:'muted', prettyms:prettyMilliseconds(clients[conn.id].timeMute - Date.now(), {verbose: true})
                }));
                 
            } else {
                clients[conn.id].muted = false
                muted.splice(i);
...
function updateUser(id, name) {
    if(name.length > 2 && name.length < 17  && !utils.checkUser(clients, name)  && name != 'Console' && name != 'System') {
...
if (data.type == "stream") {
    ...
    switch(data.type) {
        ...
        case 'global': case 'kick': case 'ban': case 'role': case 'mute': case 'unmute':  
            if(user.role > 0) {
                if(data.type == 'global') {
                    if(user.role == 3) {
                        return utils.sendToAll(clients, data);
                                ...
                                case 'ban':
                                        ...
                                        if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                            for(var client in clients) {
                                                if(clients[client].un == data.message) {
                                                    bans.push([clients[client].ip, Date.now(), Date.now() +time * 1000 * 60]);
                                                }
                                            }
                                    ...
                                    if(!isNaN(time) && time > 0) {
                                        if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                            for(var client in clients) {
                                                if(clients[client].un == data.message) {
                                                    muted.push([clients[client].ip, Date.now(), Date.now() +time * 1000 * 60]);
                                                  clients[client].muted = true
                                                  clients[client].timeMute = Date.now() +time * 1000 * 60
                                                }
                                            }

                                            data.extra = data.message;
                                            data.message = data.user + ' muted ' + data.message + ' from the server for ' + time + ' minutes';
                                            data.mid = (Math.random() + 1).toString(36).substr(2, 5);
                                        ...
                                        data.type = 'light';
                                        data.message = 'Use /mute [user] [minutes]';
                                        return utils.sendToOne(clients, users, data, data.user, 'message')
                                    }
                                    break;
...
case 'unmute':
                                        ...
                                        if(user.role > 1 && utils.getUserByName(clients, data.message).role == 0) {
                                            for(var client in clients) {
                                                if(clients[client].un == data.message) {
                                                  clients[client].muted = false
                                                }
                                            }

                                            data.extra = data.message;
                                            data.message = data.user + ' unmuted ' + data.message + ' from the server';
                                            data.mid = (Math.random() + 1).toString(36).substr(2, 5);
                                            return utils.sendToAll(clients, data);
                                        } else {
                                            data.message = 'You don\'t have permission to do that';
