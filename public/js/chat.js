 ...
 if (data.type == 'stream') {
    ...
    return showChat('global', data.user, data.message, data.subtxt, data.mid)
        ...
        if(data.type == 'server') {
            switch(data.info) {
                case 'rejected':
                    ...
                    return showChat('light', null, 'You have been muted from the server for ' + data.prettyms + ' You have to wait until you get unmuted to be able to chat again');
               ...
               location.reload();
        }else if (data.type == "mute" &&  data.extra == username ) {
          showChat('global', null, 'You have muted ');
                    updateBar('mdi-content-send', 'You muted!', false);
        }else if (data.type == "unmute" &&  data.extra == username ) {
          showChat('global', null, 'You have unmuted ');
                    updateBar('mdi-content-send', 'Enter your message here', false);
        } else {
        ...
        if(data.type == 'global' || data.type == 'pm' || data.type == 'mention') {
            if(!focus) {
                unread++;
                document.title = '(' + unread + ') Sanalika Chat';
...
function showChat(type, user, message, subtxt, mid) {
    ...
    if(type == 'global' || type == 'kick' || type == 'ban' || type == 'info' || type == 'light' || type == 'help' || type == 'role' || type == 'mute' || type == 'unmute') {
        user = 'System';
    }
    ...
    if(value.length > 0) {
        if(username === undefined) {
            ...
            switch(command[0].toLowerCase()) {
                case 'pm': case 'msg': case 'role': case 'kick': case 'ban': case 'name': case 'alert': case 'me': case 'em': case 'mute': case 'unmute':  
                    if(value.substring(command[0].length).length > 1) {
                        ...
                        if(command[0] == 'ban' && value.substring(command[0].concat(command[1]).length).length > 2) {
                         ...
                         if(command[0] == 'mute' && value.substring(command[0].concat(command[1]).length).length > 2) {
                            sendSocket(command[1], 'mute', command[2]);
                        } else if(command[0] == 'mute') {
                            showChat('light', 'Error', 'Use /mute [user] [minutes]');
                        }
                        ...
                        if(command[0] == 'kick' || command[0] == 'me' || command[0] == 'unmute' || command[0] == 'em') {
                ...
                case 'help':
                    ...
                    case 'stream':
                    sendSocket(value.substring(7), 'stream');
                    break;
